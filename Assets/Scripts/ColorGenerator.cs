...
public class ColorGenerator {
    ...
    public void UpdateSettings(ColorSettings settings) {
        ...
        if (_texture == null || _texture.height != _settings.biomeColorSettings.biomes.Length) {
            _texture = new Texture2D(TextureResolution * 2, _settings.biomeColorSettings.biomes.Length,
    ...
    public float BiomePercentFromPoint(Vector3 pointOnUnitSphere) {
        ...
        heightPercent += (biomeNoiseFilter.Evaluate(pointOnUnitSphere) - _settings.biomeColorSettings.noiseOffset) *
        ...
        foreach (var biome in _settings.biomeColorSettings.biomes) {
            for (int i = 0; i < TextureResolution * 2; i++) {
                ...
                if (i < TextureResolution) {
                    gradientColor = _settings.oceanColor.Evaluate(i / (TextureResolution - 1f));
                } else {
